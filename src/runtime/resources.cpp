...
Resource* Resource::GetInstance() { return &mResource; }
...
void Resource::Initialize() {
    ...
    nativeEmitters.clear(); nativeTextures.clear();
...
ResEmitter Resource::FindEmitter(const char* name,EffectProject* project) const { return ResEmitter(_FindEmitter(name,project)); }
ResTexture Resource::FindTexture(const char* name,TextureProject* project) const { return ResTexture(_FindTexture(name,project)); }
u32 Resource::NumEmitter(EffectProject*) const { return nativeEmitters.size(); }
EmitterResource* Resource::_GetEmitterIndexOf(u32 index,EffectProject*) const {
    if (index>=nativeEmitters.size()) return nullptr;
    auto it=nativeEmitters.begin(); std::advance(it,index); return it->second;
}
}
