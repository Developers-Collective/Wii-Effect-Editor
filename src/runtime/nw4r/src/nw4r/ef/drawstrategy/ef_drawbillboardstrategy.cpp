...
static u8 billboard_tex0_u8[] = {0x00, 0x01, 0x00, 0x00, 0x01, 0x00, 0x01, 0x01};

DrawBillboardStrategy::DrawBillboardStrategy() {
}

void DrawBillboardStrategy::Draw(const DrawInfo& rInfo, ParticleManager* pManager) {
...
void DrawBillboardStrategy::DrawNormalBillboard(const DrawInfo& rInfo, ParticleManager* pManager) {
    ...
    f32 vx = math::FSqrt(viewMtx._00 * viewMtx._00 + viewMtx._10 * viewMtx._10 + viewMtx._20 * viewMtx._20);

    f32 vy = math::FSqrt(viewMtx._01 * viewMtx._01 + viewMtx._11 * viewMtx._11 + viewMtx._21 * viewMtx._21);
    ...
    mag = math::FSqrt(rInfoMtx._01 * rInfoMtx._01 + rInfoMtx._11 * rInfoMtx._11);
    if (mag == 0.0f) {
        rs = 0.0f;
        rc = 1.0f;
    } else {
    ...
    for (Particle* pIt = pGetFirstFunc(pManager); pIt != NULL; pIt = pGetNextFunc(pManager, pIt)) {
        ...
        DispParticle_Normal(pIt, viewMtx, vx, vy, vz, rc, rs, sx, sy, pivot, flags);
    }
}

inline void DrawBillboardStrategy::DispParticle_Normal(Particle* pParticle, const math::MTX34& rViewMtx, f32 vx, f32 vy,
                                                       f32 vz, f32 rc, f32 rs, f32 sx, f32 sy, const math::VEC2& rPivot,
                                                       int flags) {
...
void DrawBillboardStrategy::DrawYBillboard(const DrawInfo& info, ParticleManager* manager) {
    ...
    math::MTX34 view;
    manager->CalcGlobalMtx(&view);
    math::MTX34Mult(&view, &camera, &view);
    if (setting.zOffset)
        CalcZOffset(&view, manager, info, setting.zOffset);
    ...
    for (auto* particle = firstParticle(manager); particle; particle = nextParticle(manager, particle)) {
        ...
        if (sx < std::numeric_limits<float>::epsilon() || sy < std::numeric_limits<float>::epsilon())
            continue;
        SetupGP(particle, setting, info, first, false);
        first = false;
        ...
        float sr = 0, cr = 1;
        if (rotation.z)
            math::SinCosRad(&sr, &cr, -rotation.z);
...
void DrawBillboardStrategy::DrawDirectionalBillboard(const DrawInfo& info, ParticleManager* manager) {
    ...
    const float vy = std::sqrt(view._01 * view._01 + view._11 * view._11 + view._21 * view._21);
    const float px = setting.pivotX / 100.f, py = setting.pivotY / 100.f;
    AheadContext context(camera, manager);
...
void DrawBillboardStrategy::DispPolygon(const math::VEC3& rP, const math::VEC3& rD1, const math::VEC3& rD2, int flags) {
...
void DrawBillboardStrategy::CalcZOffset(math::MTX34* pMtx, const ParticleManager* pManager, const DrawInfo& rInfo,
                                        f32 offsetZ) {
...
void DrawBillboardStrategy::InitGraphics(const DrawInfo& rInfo, ParticleManager* pManager) {
...
DrawStrategyImpl::CalcAheadFunc DrawBillboardStrategy::GetCalcAheadFunc(ParticleManager* pManager) {
