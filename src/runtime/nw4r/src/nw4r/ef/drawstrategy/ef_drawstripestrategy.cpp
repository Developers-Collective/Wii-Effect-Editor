...
void DrawStripeStrategy::Draw(const DrawInfo& info, ParticleManager* manager) {
    DrawStripes(info, manager, false);
}
void DrawSmoothStripeStrategy::Draw(const DrawInfo& info, ParticleManager* manager) {
    DrawStripes(info, manager, true);
}
...
DrawStrategyImpl::CalcAheadFunc DrawStripeStrategy::GetCalcAheadFunc(ParticleManager*) {
    return CalcAhead_Stripe;
}
DrawStrategyImpl::CalcAheadFunc DrawSmoothStripeStrategy::GetCalcAheadFunc(ParticleManager*) {
    return CalcAhead_Stripe;
}
void DrawStrategyImpl::CalcAhead_Stripe(math::VEC3* result, AheadContext* context, Particle* p) {
    ...
    switch (s.typeDir) {
    case 1:
        CalcAhead_EmitterCenter(result, context, p);
        return;
    case 2:
        CalcAhead_EmitterDesign(result, context, p);
        return;
    case 5:
    case 7:
        CalcAhead_NoDesign(result, context, p);
        return;
    case 3: {
    ...
    case 6: {
        auto* elder = GetElderDrawParticle(manager, p);
        auto* younger = GetYoungerDrawParticle(manager, p);
        const int connection = s.typeOption2 & 7;
        if (connection == 1) {
            if (!elder)
                elder = GetYoungestDrawParticle(manager);
            if (!younger)
                younger = GetOldestDrawParticle(manager);
        }
        math::VEC3 a(0, 0, 0), b(0, 0, 0);
        if (elder) {
            a = elder->mParameter.mPosition - p->mParameter.mPosition;
            Normalize(&a);
        }
        if (younger) {
            b = younger->mParameter.mPosition - p->mParameter.mPosition;
            Normalize(&b);
        } else if (connection == 2) {
            b = context->mCommon.mEmitterCenter - p->mParameter.mPosition;
            Normalize(&b);
        }
        *result = a - b;
        break;
    ...
    default:
        CalcAhead_Speed(result, context, p);
        return;
    }
    if (!Normalize(result))
        *result = context->mCommon.mEmitterAxisY;
}
void DrawStrategyImpl::DrawStripes(const DrawInfo& info, ParticleManager* manager, bool smooth) {
    ...
    case 8:
        initial = column(axes, 0);
        break;
    case 16:
        initial = column(axes, 2);
        break;
    case 24:
        initial = column(axes, 0) + column(axes, 1) + column(axes, 2);
        break;
    default:
        initial = column(axes, 1);
        break;
    }
    if (!Normalize(&initial))
        initial = context.mCommon.mEmitterAxisY;
    for (auto* p = GetYoungestParticle(manager); p && p->mPrevAxis.x > 1; p = GetElderParticle(manager, p))
        p->mPrevAxis = initial;
    math::VEC3 screenZ(0, 0, 1);
    if (math::MTX34Inv(&inverseView, &modelView))
        screenZ = column(inverseView, 2);
    auto makeVertex = [&](Particle* p, math::VEC3 pos, float t) {
        math::VEC3 y, x, z;
        CalcAhead_Stripe(&y, &context, p);
        auto previous = billboard ? screenZ : p->mPrevAxis;
        math::VEC3Cross(&x, &y, &previous);
        if (!Normalize(&x))
            x = fallback;
        ...
        p->mPrevAxis = z;
        math::VEC3 rot;
        p->Draw_GetRotate(&rot);
        float sn, cs;
        math::SinCosRad(&sn, &cs, rot.y);
    ...
    if (particles.size() < minimum || (tube && s.typeOption0 < 3)) {
        for (auto* p : particles)
            makeVertex(p, p->mParameter.mPosition, 0);
        return;
    }
    InitTexture(s);
    InitTev(s, info);
    ...
    if (connection == 2 && !reverse)
        control.push_back(makeVertex(youngest, context.mCommon.mEmitterCenter, 0));
    for (int i = 0; i < n; ++i) {
        float t = dt * float(reverse ? total - 1 - i : i + (connection == 2));
        if (smooth && connection == 1)
            t = dt * (reverse ? n + .5f - i : i - .5f);
        control.push_back(makeVertex(particles[i], particles[i]->mParameter.mPosition, t));
    }
    if (connection == 2 && reverse)
        control.push_back(makeVertex(youngest, context.mCommon.mEmitterCenter, 0));
    if (connection == 1) {
        auto closing = control.front();
        closing.t = dt * (reverse ? 0 : n);
        if (smooth)
            closing.t = dt * (reverse ? .5f : n - .5f);
    ...
    if (smooth) {
        ...
        if (connection != 1) {
            control.insert(control.begin(), control.front());
            control.push_back(control.back());
        }
        for (size_t i = 0; i + 2 < control.size(); ++i) {
            for (int j = 0; j < subdivisions; ++j)
                vertices.push_back(interpolate(control[i], control[i + 1], control[i + 2], float(j) / subdivisions));
        }
        const size_t end = control.size();
        vertices.push_back(interpolate(control[end - 3], control[end - 2], control[end - 1], 1));
    } else
        vertices = std::move(control);
    auto emit = [&](math::VEC3 pos, float u, float v) {
    ...
    if (tube) {
        ...
        for (int j = 0; j <= sides; ++j) {
            float sn = 0, cs = 1;
            if (j && j < sides)
                math::SinCosFIdx(&sn, &cs, 256.f * float(j) / sides);
        ...
        for (auto cull : {GX_CULL_FRONT, GX_CULL_BACK}) {
            ...
            for (size_t i = 1; i < vertices.size(); ++i) {
                ...
                for (int j = 0; j <= sides; ++j) {
                    const auto& a = vertices[reverse ? i - 1 : i];
                    const auto& b = vertices[reverse ? i : i - 1];
        ...
        for (int plane = 0; plane < (s.typeOption == 1 ? 2 : 1); ++plane) {
            ...
            for (size_t first = 0; first + 1 < vertices.size();) {
                ...
                for (size_t i = first; i < end; ++i) {
                    ...
                    auto axis = plane ? v.z : v.x;
                    if (plane && !smooth)
                        axis *= -1;
                    emit(v.center + axis, 1, v.t);
                    emit(v.center - axis, 0, v.t);
                }
                GXEnd();
