...
MTX34* MTX34RotXYZFIdx(MTX34* out,f32 fx,f32 fy,f32 fz) {
    ...
    SinCosFIdx(&sx,&cx,fx); SinCosFIdx(&sy,&cy,fy); SinCosFIdx(&sz,&cz,fz);
    out->_20=-sy; out->_00=cz*cy; out->_10=sz*cy;
    out->_21=cy*sx; out->_22=cy*cx;
    f32 cx_sz=cx*sz,sx_cz=sx*cz,sx_sz=sx*sz,cx_cz=cx*cz;
    out->_01=std::fma(sx_cz,sy,-cx_sz); out->_12=std::fma(cx_sz,sy,-sx_cz);
    out->_02=std::fma(cx_cz,sy,sx_sz); out->_11=std::fma(sx_sz,sy,cx_cz);
