...
struct Bytes {
    ...
    void check(size_t p, size_t n) const {
        if (p > data.size() || n > data.size() - p)
            throw std::runtime_error("Truncated animation curve");
    }
...
Time curveTime(const Bytes& b, uint32_t tick, uint32_t life) {
    ...
    if (!infinite && loops < 2) {
        if (!fit)
            return {float(std::min(tick, last)), 0};
        return {life == 1 ? float(last) : std::min(float(last), float(tick) * (float(last) / float(life - 1))), 0};
    }
    if (fit) {
        if (tick >= life - 1)
            return {turn && !(loops & 1) ? 0.f : float(last), uint8_t(loops - 1)};
...
uint32_t nextRandom(uint32_t state) {
    return state * 0x343fdU + 0x269ec3U;
}
...
Time discreteTime(const Bytes& b, uint32_t tick, uint32_t life) {
    ...
    if (!length || !life)
        throw std::runtime_error("Zero texture animation duration");
    const bool infinite = flags & 32, turn = flags & 64, fit = flags & 128;
    if (!infinite && loops < 2)
        return {float(uint16_t(fit ? (length * tick) / life : std::min(tick, length))), 0};
    if (fit) {
        if (tick >= life)
            return {turn && !(loops & 1) ? 0.f : float(length), uint8_t(loops - 1)};
        if (!turn || length < 2) {
        ...
        if (loop >= loops)
            return {loops & 1 ? float(last) : 0.f, uint8_t(loops - 1)};
    ...
    return {float(uint16_t(turn && length >= 2 && (loop & 1) ? period * (loop + 1) - tick : tick - loop * period)),
            loop};
}
...
template <class T, bool rotate = false>
void evaluateNumeric(std::span<const uint8_t> curve, std::span<T> output, uint32_t tick, uint16_t seed, uint32_t life) {
    ...
    if (!mask)
        return;
    if (output.size() < std::bit_width(unsigned(mask)))
        throw std::runtime_error("Animation target is too small");
    ...
    if (b.u8(0) == 0xAB) {
        ...
        for (unsigned c = 0; c < 8; ++c)
            if (mask & (1 << c)) {
                output[c] = bytes ? T(b.u8(value)) : T(b.f32(value));
                value += sizeof(T);
            }
        return;
    }
    if (b.u8(0) != 0xAC)
        throw std::runtime_error("Unknown animation magic");
    const size_t count = b.u16(32), stride = bytes ? ((13 + components) & ~size_t(1)) : 12 + components * 4;
    if (!count)
        throw std::runtime_error("Animation has no keys");
    b.check(36, count * stride);
    size_t left = 0;
    // Keys are ordered by integer frame. Fractional fitted times interpolate.
    while (left + 1 < count && float(b.u16(36 + (left + 1) * stride)) <= time.frame)
        ++left;
    ...
    if (!exact && (flags & 64) && ((flags & 32) || loops > 1)) {
        if (!(time.loop & 1) && left + 2 >= count && ((flags & 32) || time.loop < uint32_t(loops - 1)))
            ++rightLoop;
        if ((time.loop & 1) && !left && time.loop)
            ++leftLoop;
    }
    auto values = [&](size_t index, uint32_t loop) {
        ...
        if (type) {
            ...
            if (type & 2) {
                ...
                if (!n)
                    throw std::runtime_error("Empty animation random table");
                value = table + 4 + ((random >> 16) % n) * (components * 2 * sizeof(T) + (rotate ? 4 : 0));
                random = nextRandom(random);
            } else
                value = table + 4 + size_t(rangeIndex) * (components * 2 * sizeof(T) + (rotate ? 4 : 0));
            if constexpr (rotate) {
        ...
        for (unsigned c = 0; c < std::bit_width(unsigned(mask)); ++c) {
            if (mask & (1 << c)) {
                if (!type) {
                    out[c] = bytes ? float(b.u8(value)) : b.f32(value);
                    value += sizeof(T);
                } else if constexpr (bytes) {
                    out[c] = float(std::clamp(
                        int(b.u8(value)) + int(b.u8(value + 1)) * int(int16_t(random >> 16)) / 32768, 0, 255));
                    value += 2;
                } else {
                    out[c] = b.f32(value) + b.f32(value + 4) * float(random >> 16);
                    if (negate)
                        out[c] = -out[c];
                    value += 8;
                }
            }
            if (type && !(flags & 4) && (bytes || rotate || (mask & (1 << c))))
                random = nextRandom(random);
        }
        return out;
    };
    ...
    for (unsigned c = 0; c < 8; ++c)
        if (mask & (1 << c)) {
            ...
            if constexpr (bytes) {
                if ((code & 3) == 0)
                    output[c] =
                        uint8_t(int(a[c]) + int(std::floor(double(int64_t(fixed) * int(z[c] - a[c])) / 65536.)));
                else
                    output[c] = uint8_t(std::clamp(interpolate(a[c], z[c], t, code), 0.f, 255.f));
            } else
                output[c] = interpolate(a[c], z[c], t, code);
        }
}
void evaluateF32(std::span<const uint8_t> b, std::span<float> out, uint32_t tick, uint16_t seed, uint32_t life) {
    evaluateNumeric(b, out, tick, seed, life);
}
void evaluateU8(std::span<const uint8_t> b, std::span<uint8_t> out, uint32_t tick, uint16_t seed, uint32_t life) {
    evaluateNumeric(b, out, tick, seed, life);
}
void evaluateRotate(std::span<const uint8_t> b, std::span<float> out, uint32_t tick, uint16_t seed, uint32_t life) {
    evaluateNumeric<float, true>(b, out, tick, seed, life);
}
TextureSelection evaluateTexture(std::span<const uint8_t> curve, uint32_t tick, uint16_t seed, uint32_t life) {
    ...
    if (!count)
        throw std::runtime_error("Texture animation has no keys");
    b.check(36, count * 16);
    size_t index = 0;
    while (index + 1 < count && b.u16(36 + (index + 1) * 16) <= time.frame)
        ++index;
    ...
    if (type) {
        ...
        if (type & 2) {
            ...
            if (!b.u16(table))
                throw std::runtime_error("Empty texture random table");
            value = table + 4 + (random % b.u16(table)) * 4;
        } else
            value = ranges + 4 + size_t(b.u16(value)) * 8;
    }
    TextureSelection out{b.u8(value), b.u8(value + 1), b.u16(value + 2)};
    if (type && !(type & 2)) {
        const auto reverse = b.u8(value + 4);
        if (reverse == 1)
            out.reverse = (out.reverse & 2) | (random & 1);
        else if (reverse == 2)
            out.reverse = (out.reverse & 1) | (random & 2);
        else if (reverse == 3)
            out.reverse = random & 3;
    }
    return out;
}
std::string curveName(std::span<const uint8_t> curve, uint16_t index) {
    ...
    if (index >= count)
        throw std::runtime_error("Animation name index out of range");
    size_t pos = table + 4 + size_t(count) * 4;
    for (unsigned i = 0; i <= index; ++i) {
        const size_t length = b.u16(pos);
        pos += 2;
        b.check(pos, length);
        if (!length || b.u8(pos + length - 1) != 0)
            throw std::runtime_error("Invalid animation name");
        if (i == index)
            return {reinterpret_cast<const char*>(curve.data() + pos), length - 1};
        pos += length;
    }
    throw std::runtime_error("Animation name not found");
}
std::vector<std::array<uint8_t, 12>> evaluateChild(std::span<const uint8_t> curve, uint32_t tick, uint16_t seed,
                                                   uint32_t life) {
    Bytes b{curve};
    b.check(0, 36);
    ...
    auto create = [&](size_t index) {
        ...
        if (type) {
            if (!(type & 2))
                throw std::runtime_error("Invalid range child key");
        ...
        std::copy_n(curve.data() + value, 12, data.begin());
        result.push_back(data);
    };
    auto forward = [&](float first, float end) {
        for (size_t i = 0; i < count; ++i) {
            const auto frame = b.u16(36 + i * 24);
    ...
    auto backward = [&](float first, float end) {
        for (size_t i = count; i > 0; --i) {
            const auto frame = b.u16(36 + (i - 1) * 24);
            if (frame <= first && frame > end)
                create(i - 1);
        }
    };
    if (current.loop == next.loop) {
        if (current.frame == next.frame) {
            ...
            if (length >= 2 && (flags & 64) && !(flags & 32) && loops >= 2 && !(loops & 1)) {
                if (flags & 128)
                    endpoint = tick == life - 1;
                else
                    endpoint = tick == loops * (length - 1);
            }
            if (endpoint)
                forward(0, 1);
        } else if (current.frame < next.frame)
            forward(current.frame, next.frame);
        else
            backward(current.frame, next.frame);
    } else if (!(flags & 64)) {
        ...
        if (current.loop & 1)
            backward(current.frame, 0);
        else
            forward(current.frame, float(length - 1));
        ...
        for (uint32_t loop = current.loop + 1; loop < next.loop; ++loop) {
            if (current.loop & 1)
                ...
                forward(0, float(length - 1));
        }
        if (next.loop & 1)
            backward(65535, next.frame);
        else
            forward(0, next.frame);
    }
    return result;
