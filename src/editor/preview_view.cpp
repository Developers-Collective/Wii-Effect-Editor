...
void drawPreviewGrid(const PreviewRect& r, const OrbitCamera& camera, bool grid, bool axes) {
    ...
    Mtx44 projection;
    C_MTXPerspective(projection, 45.f, r.width / r.height, .001f, 1000000.f);
    ...
    auto vertex = [](float x, float y, float z, GXColor c) {
    ...
    if (grid) {
        ...
        for (int i = -10; i <= 10; ++i) {
            ...
            vertex(float(i * 10), -100, 0, color);
            vertex(float(i * 10), 100, 0, color);
            vertex(-100, float(i * 10), 0, color);
            vertex(100, float(i * 10), 0, color);
        }
        GXEnd();
    }
    if (axes) {
        ...
        vertex(0, 0, 0, {210, 85, 85, 255});
        vertex(25, 0, 0, {210, 85, 85, 255});
        vertex(0, 0, 0, {85, 190, 130, 255});
        vertex(0, 25, 0, {85, 190, 130, 255});
        vertex(0, 0, 0, {95, 145, 230, 255});
        vertex(0, 0, 25, {95, 145, 230, 255});
        GXEnd();
    }
}
