...
install(
    ...
    COMPONENT EffectEditor)
install(
    FILES "${CMAKE_CURRENT_SOURCE_DIR}/runtime/nw4r/LICENSE"
    DESTINATION licenses/nw4r
    COMPONENT EffectEditor)
foreach(
    _dependency
    aurora
    ...
    if(DEFINED ${_dependency}_SOURCE_DIR)
        file(
            ...
            "${${_dependency}_SOURCE_DIR}/LICENSE*" "${${_dependency}_SOURCE_DIR}/COPYING*")
        if(_licenses)
            install(
                FILES ${_licenses}
...
foreach(_source IN LISTS _dependency_sources)
    ...
    file(
        GLOB _licenses
        LIST_DIRECTORIES FALSE
        "${_source}/LICENSE*" "${_source}/COPYING*" "${_source}/COPYRIGHT*")
    if(_licenses)
        install(
            FILES ${_licenses}
            DESTINATION "licenses/${_name}"
            COMPONENT EffectEditor)
    endif()
    if(IS_DIRECTORY "${_source}/licenses")
        install(
            DIRECTORY "${_source}/licenses/"
            DESTINATION "licenses/${_name}"
            COMPONENT EffectEditor)
    endif()
endforeach()
