...
int main(int argc, char** argv) {
    try {
        if (argc < 3) {
            std::cout
                << "breff_converter decode|encode|decode_raw|encode_raw SOURCE [-d DESTINATION] [--version 7..11] [-o]\n";
            return argc == 1 ? 0 : 1;
        }
        const std::string operation = argv[1];
        const fs::path source = fs::u8path(argv[2]);
        fs::path destination;
        bool overwrite = false;
        unsigned version = 0;
        for (int i = 3; i < argc; ++i) {
            const std::string option = argv[i];
            if (option == "-o" || option == "--overwrite")
                overwrite = true;
            else if ((option == "-d" || option == "--dests") && i + 1 < argc)
                destination = fs::u8path(argv[++i]);
            else if (option == "--version" && i + 1 < argc)
                version = unsigned(std::stoul(argv[++i]));
            else
                throw std::runtime_error("Unknown or incomplete option: " + option);
        }
        if (version && (version < 7 || version > 11))
            throw std::runtime_error("Destination version must be 7 through 11");
        if (destination.empty()) {
            destination = source;
            if (operation == "decode")
                destination.replace_extension(".breff.d");
            else if (operation == "encode") {
                destination.replace_extension();
                destination.replace_extension(".breff");
            } else if (operation == "decode_raw")
                destination.replace_extension(".json");
            else if (operation == "encode_raw")
                destination.replace_extension();
        }
        if (operation == "decode") {
            auto archive = ResourceFile::decode(readFile(source));
            if (archive.magic != "REFF")
                throw std::runtime_error("Expected a BREFF archive");
            ...
            for (const auto& entry : archive.entries) {
                if (!validFilename(entry.name) || entry.name == "meta")
                    throw std::runtime_error("Effect name cannot be exported as a JSON filename: " + entry.name);
                effects.emplace_back(entry.name, decodeEffect(entry.data, archive.version));
            }
            fs::create_directories(destination);
            writeJson(destination / "meta.json", {{"version", archive.version}, {"projectName", archive.projectName}},
                      overwrite);
            for (const auto& [name, value] : effects)
                writeJson(destination / (name + ".json"), value, overwrite);
        } else if (operation == "encode") {
            const auto meta = readJson(source / "meta.json");
            ResourceFile archive;
            archive.version = uint16_t(version ? version : meta.at("version").get<unsigned>());
            archive.projectName = meta.at("projectName");
            std::vector<fs::path> files;
            for (const auto& file : fs::directory_iterator(source))
                if (file.is_regular_file() && file.path().extension() == ".json" &&
                    file.path().filename() != "meta.json")
                    files.push_back(file.path());
            std::sort(files.begin(), files.end());
            for (const auto& file : files)
                archive.entries.push_back({file.stem().string(), encodeEffect(readJson(file), archive.version)});
            writeFile(destination, archive.encode(), overwrite);
        } else if (operation == "decode_raw")
            writeJson(destination, decodeEffect(readFile(source), version ? version : 11), overwrite);
        else if (operation == "encode_raw")
            writeFile(destination, encodeEffect(readJson(source), version ? version : 11), overwrite);
        else
