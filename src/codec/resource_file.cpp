...
ResourceFile ResourceFile::decode(std::span<const uint8_t> bytes) {
    ...
    if ((result.magic != "REFF" && result.magic != "REFT") || r.at(4, 2) != 0xfeff || r.at(8, 4) != bytes.size() ||
        r.at(12, 2) != 16 || r.at(14, 2) != 1 || r.at(16, 4) != r.at(0, 4))
...
Bytes ResourceFile::encode() const {
    ...
    w.patch(8, w.bytes.size(), 4);
    w.patch(20, w.bytes.size() - 24, 4);
